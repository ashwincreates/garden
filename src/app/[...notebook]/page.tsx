...
async function Note({ params }: { params: { notebook: string[] } }) {
  ...
  const content = await getNote(notebook).catch(() => {
    notFound();
  });
  ...
  return (
      <div className="min-h-screen bg-[#f5f2ed]" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        <div className="md:ml-[16rem]">
 
          {/* Top bar */}
          <div
            className="sticky top-0 z-10 flex items-center gap-2 px-8 h-[52px] border-b border-[#e2ddd5]"
            style={{ background: "rgba(245,242,237,0.92)", backdropFilter: "blur(8px)" }}
          >
            ...
            <span className="text-[#d4cfc6] text-sm">/</span>
            <span className="text-[#3d3a34] text-sm font-medium capitalize">{title}</span>
          </div>
          ...
          <div className="max-w-[680px] mx-auto px-8 py-14">
            ...
            <header className="mb-10 pb-8 border-b border-[#e2ddd5]">
              ...
              >
                ...
                <span className="text-[10px] text-[#b8b4ad] uppercase tracking-[0.18em]">.md</span>
              </div>
              <h1
